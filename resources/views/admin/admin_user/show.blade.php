      ...
      <div class="col-12">
         <div class="card bg-light">
            ...
            <div class="card-body pt-0">
               <div class="row">
                  <div class="col-3">
                     <h2 class="">{{ __(ucwords($users->user_profiles->first_name .' '.$users->user_profiles->last_name)) }}</h2>
                     ...
                     <p class="text-muted text-md"><b>State : </b> {{ __($users->user_profiles->states ? $users->user_profiles->states->name : 'NA')}}  </p>
                  ...
                  <div class="col-3">
                     ...
                     <p class="text-muted text-md"><b>Date Of Birth : </b> {{ __($users->user_profiles->dob)}}  </p>
                     <p class="text-muted text-md"><b>Relationship : </b> {{ __($users->user_profiles->relationship)}}  </p>
                     <p class="text-muted text-md"><b>Suburb : </b> {{ __($users->user_profiles->suburb)}}  </p>
                     <p class="text-muted text-md"><b>Preferred Location : </b> {{ __($users->user_profiles->preferred_location)}}  </p>
                     <p class="text-muted text-md"><b>Address : </b> {{ _($users->user_profiles->address ? $users->user_profiles->address :  'NA') }}  </p>
                     
                  </div>
                  <div class="col-4 text-center">
                     @if($users->profile_photo_path)
                     <a href="{{ asset('storage/'.$users->profile_photo_path) }}" data-toggle="lightbox" data-title="{{ __(ucwords($users->user_profiles->first_name .' '.$users->user_profiles->last_name)) }}" data-gallery="gallery">
                      <img src="{{ asset('storage/'.$users->profile_photo_path) }}" class="img-circle img-fluid" height="40%" width="40%" alt="Profile Photo"/>
                     </a>                     
                     @else
